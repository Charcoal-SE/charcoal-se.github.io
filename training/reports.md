...
## Flagging
You should be familiar with this since you have experience on Stack Exchange. Follow the link to look 
at the post, decide if it needs flagging, and if so, flag it with the appropriate flag.
...
feedback: true positives, false positives, and NAA. You should feed back as follows:
 ...
 - If the reported post is **spam or abusive**, it's a true positive. Reply to the report
   message with `tpu`.
 - If the reported post is Not An Answer, reply to the report with `naa`.
 - If the reported post is self-vandalism, reply to the report with `tp`.
 - If the reported post is a legitimate post, reply to the report with `fp`.
...
leaving a comment, it is recommended that you raise a "looked at by a moderator" flag
...
network policy, it ended up causing considerable disruption and was of great concern to the
