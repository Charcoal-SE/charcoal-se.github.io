...
(function() {
  ...
  $(document).on('ready turbolinks:load', function() {
    var _closeIfNeeded, closeSearch, id, index, openSearch, page, ref, search, term;
    search = function(term) {
      ...
      if (results.length) {
        content = results.map(function(res) {
          var info;
      ...
      return $('.search .link').attr('href', '/search?q=' + encodeURIComponent(term).replace(/%20/g, '+'));
    };
    openSearch = function() {
      $('.search').fadeIn('fast');
      $('.search-query').focus();
      return $(document).on('keyup', _closeIfNeeded);
    };
    closeSearch = function() {
      $('.search').fadeOut('fast');
      return $(document).off('keyup', _closeIfNeeded);
    };
    _closeIfNeeded = function(e) {
      if (e.which === 27) {
        return closeSearch();
      }
    };
    ...
    if (location.search.substring(0, 3) === '?q=') {
      term = decodeURIComponent(location.search.substring(3).replace(/\+/g, '%20'));
    }
    if (term) {
    ...
    return $('.search-query').on('keyup', function() {
      return search($('.search-query').val());
    });
  });
